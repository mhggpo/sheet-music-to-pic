...
class SheetParser:
    ...
    def source_parser(self, sheet: str) -> List[List[tuple]]:
        res = []
        try:
            temp = sheet.strip().split(' ')
    ...
    def sheet_parser(self, sheet: str, of: int = 0) -> List[List[str]]:
        res = []
        try:
            temp = sheet.strip().split(' ')
        except AttributeError or SyntaxError:
    ...
    def array_parser(self, arr: List[List[str]]) -> str:
        if not arr:
            return ''
        res = "\n"
        for k in arr:
...
if __name__ == '__main__':
    ...
    p = SheetParser()
    t = p.sheet_parser(test_sheet1, 2)
    print(t)
    print(p.source_parser(test_sheet1))
    print(p.array_parser(t))
